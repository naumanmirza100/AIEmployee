...
class Ticket(models.Model):
    ...
    # Free-form labels orthogonal to `category` — useful for topic/team/product
    # cross-cutting tags that don't fit the category enum. List of short strings,
    # tags are unique per ticket and matched case-sensitively for filtering.
    tags = models.JSONField(
        default=list, blank=True,
        help_text='List of free-form short labels. Independent of the `category` enum.',
    )
...
class NotificationTemplate(models.Model):
    ...
    # Per-template quiet hours — per-user preferences exist on
    # ``FrontlineNotificationPreferences`` but you can't currently say "this
    # template is urgent, ignore quiet hours" OR "this newsletter respects a
    # different window than the user's default". Shape:
...
class Document(models.Model):
    ...
    # Soft deprecation — set to True when HR/CS wants to take a doc out of
    # retrieval without uploading a replacement (e.g. "this policy is wrong,
    # we'll publish a corrected version next week"). Different from
    # `superseded_by` because there's no replacement row.
...
class DocumentChunk(models.Model):
    ...
    page_number = models.PositiveIntegerField(
        ...
        help_text='1-based PDF page this chunk came from. Null for non-PDF docs or PDF chunks that span page breaks.',
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class TicketLink(models.Model):
    """Typed relationship between two tickets. Lets agents surface dependency
    chains ("blocks", "duplicate of", "related to") instead of stuffing
    references in description text.

    The relation is directional — `from_ticket` is the one being annotated
    (the agent says "this ticket is a duplicate OF that one"). Querying
    "what links into me?" goes via `linked_from`.
    """
    RELATION_CHOICES = [
    ...
    from_ticket = models.ForeignKey(
        'Ticket', on_delete=models.CASCADE, related_name='linked_to',
        help_text='The ticket being annotated.',
    )
    to_ticket = models.ForeignKey(
        'Ticket', on_delete=models.CASCADE, related_name='linked_from',
        help_text='The ticket on the other end of the relationship.',
    )
    relation = models.CharField(max_length=20, choices=RELATION_CHOICES)
    created_by = models.ForeignKey(
        'core.CompanyUser', on_delete=models.SET_NULL,
        null=True, blank=True, related_name='frontline_ticket_links_created',
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['from_ticket', 'relation']),
            models.Index(fields=['to_ticket', 'relation']),
        ]

    def __str__(self):
        return f"#{self.from_ticket_id} {self.relation} #{self.to_ticket_id}"


class ContactNote(models.Model):
    """Free-form note attached to a Contact, separate from per-ticket notes.

    Use case: "VIP — escalate immediately if they call", "Prefers email, not
    phone", etc. Notes are scoped to the contact's company via the FK. Pinned
    notes float to the top in the UI; ordering otherwise is reverse-chrono.
    """
    contact = models.ForeignKey(
        'Frontline_agent.Contact', on_delete=models.CASCADE,
        related_name='notes',
    )
    body = models.TextField()
    ...
    created_by = models.ForeignKey(
        'core.CompanyUser', on_delete=models.SET_NULL,
        null=True, blank=True, related_name='frontline_contact_notes_created',
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['contact', '-is_pinned', '-created_at']),
        ]

    def __str__(self):
        return f"ContactNote(contact={self.contact_id}, pinned={self.is_pinned})"
