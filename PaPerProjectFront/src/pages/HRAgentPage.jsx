...
const HRAgentPage = () => {
  ...
  const [activeSection] = useState('hr');
  ...
  useEffect(() => {
    ...
    if (!companyUserStr) {
      toast({
        title: 'Not logged in',
        description: 'Please log in to access the HR Support Agent',
        variant: 'destructive',
      });
  ...
  const checkModuleAccessForUser = async () => {
    try {
      ...
      const response = await checkModuleAccess('hr_agent');
      if (response.status === 'success') {
        setHasAccess(response.has_access);
        if (!response.has_access) {
          toast({
            title: 'Module Not Purchased',
            description: 'Please purchase the HR Support Agent module to access this dashboard',
            variant: 'default',
          });
        }
      }
          ...
          <Card className="max-w-md w-full">
            <CardHeader>
              ...
              <CardDescription className="text-center">
                You need to purchase the HR Support Agent module to access this dashboard.
              </CardDescription>
            </CardHeader>
  ...
  return (
    <>
      <Helmet>
        <title>HR Support Agent | Pay Per Project</title>
      </Helmet>
      <div
      ...
      >
        <DashboardNavbar
          icon={Users}
          title={companyUser.companyName || 'HR Support Agent'}
          ...
          navItems={getAgentNavItems(purchasedModules, 'hr', navigate)}
        />
