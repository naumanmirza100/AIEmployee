...
const ExecMeetingAgentPage = () => {
  ...
  const [activeSection] = useState('exec-meeting');
  ...
  useEffect(() => {
    ...
    if (!companyUserStr) {
      toast({
        title: 'Not logged in',
        description: 'Please log in to access the Executive Meeting Assistant',
        variant: 'destructive',
      });
  ...
  const checkModuleAccessForUser = async () => {
    try {
      ...
      const response = await checkModuleAccess('exec_meeting_agent');
      if (response.status === 'success') {
        setHasAccess(response.has_access);
        if (!response.has_access) {
          toast({
            title: 'Module Not Purchased',
            description: 'Please purchase the Executive Meeting Assistant module to access this dashboard',
            variant: 'default',
          });
        }
      }
          ...
          <Card className="max-w-md w-full">
            <CardHeader>
              ...
              <CardDescription className="text-center">
                You need to purchase the AI Executive Meeting Assistant module to access this dashboard.
              </CardDescription>
            </CardHeader>
  ...
  return (
    <>
      <Helmet>
        <title>AI Executive Meeting Assistant | Pay Per Project</title>
      </Helmet>
      <div
      ...
      >
        <DashboardNavbar
          icon={CalendarClock}
          title={companyUser.companyName || 'AI Executive Meeting Assistant'}
          ...
          navItems={getAgentNavItems(purchasedModules, 'exec-meeting', navigate)}
        />
...
export default ExecMeetingAgentPage;
