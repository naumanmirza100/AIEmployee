 ...
 *
 * XHR-based so `options.onProgress({ loaded, total, percent })` reports real
 * upload-byte progress. Returns 202 immediately with a `summary.id` you can
 * poll via `getSummaryStatus` — summarising a large doc takes several LLM
 * calls, so the work happens in the background.
 */
export const uploadAndSummarize = async (file, options = {}) => {
  const token = localStorage.getItem('company_auth_token');
  const fd = new FormData();
  fd.append('file', file);
  ...
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', `${API_BASE_URL}/operations/summaries/upload/`, true);
    if (token) xhr.setRequestHeader('Authorization', `Token ${token}`);
    ...
    xhr.onload = () => {
      ...
      if (data.status === 'error') {
        const err = new Error(data.message || 'Summarization failed');
        err.status = xhr.status;
        err.data = data;
        return reject(err);
      }
...
/**
 * Poll summarisation progress for a single summary. Returns
 * { processing_status, percent, processing_error, is_indexed }.
 */
export const getSummaryStatus = async (summaryId) => {
  try {
    const response = await companyApi.get(`/operations/summaries/${summaryId}/status`);
    return response;
  } catch (error) {
