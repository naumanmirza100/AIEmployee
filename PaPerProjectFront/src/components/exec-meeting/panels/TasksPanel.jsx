...
export const TasksPanel = ({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Task detail card pop-up (opens when a task row is clicked) ── */}
      {(() => {
        const t = Array.isArray(tasks) ? tasks.find(x => x.id === expandedTaskId) : null;
        const subtasks = t?.subtasks || [];
        const subtaskDone = t?.subtask_done_count || 0;
        const subtaskTotal = t?.subtask_count ?? subtasks.length;
        const pct = subtaskTotal > 0
          ? Math.round(
              (subtasks.reduce((sum, st) => sum + (st.status === 'done' ? 1 : (st.status === 'in_progress' || st.status === 'review') ? 0.5 : 0), 0) / subtaskTotal) * 100
            )
          : 0;
                  ...
                  <div className="flex items-start justify-between px-6 py-4 pr-14 border-b border-white/10 flex-shrink-0">
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                        <span className="text-white/40 text-xs">{t.due_date ? `Due: ${t.due_date}` : 'No due date'}</span>
                        {priorityBadge(t.priority)}
                        {statusBadge(t.status)}
                      </div>
                    </div>
                    <div className="flex items-center gap-0.5 flex-shrink-0 ml-3">
                      <HoverTip tip="Edit this task">
                        <button type="button" onClick={() => setEditingTask(t)}
                          className="p-1.5 rounded text-white/40 hover:text-violet-300 hover:bg-violet-500/10 transition-colors">
                          <Pencil className="h-4 w-4" />
                        </button>
                      </HoverTip>
                      <HoverTip tip="Add a subtask">
                        <button type="button" onClick={() => setSubtaskParentTask(t)}
                          className="p-1.5 rounded text-white/40 hover:text-sky-300 hover:bg-sky-500/10 transition-colors">
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar px-6 py-5 space-y-4">
                    {t.description && (
                      <div>
                        ...
                        <p className="text-white/70 text-sm whitespace-pre-wrap">{t.description}</p>
                      </div>
                    )}
                    {(t.assignees || []).length > 0 && (
                      <div>
                        <p className="text-white/40 text-xs uppercase tracking-wide mb-1.5">Assignees</p>
                    ...
                    {t.ai_reasoning && (
                      <div>
                        <p className="text-white/40 text-xs uppercase tracking-wide mb-1">AI reasoning</p>
                        <p className="text-white/50 text-xs italic">{t.ai_reasoning}</p>
                      </div>
                    )}
                    ...
                    {!t.description && !(t.assignees || []).length && !t.ai_reasoning && subtasks.length === 0 && (
                      <p className="text-white/30 text-sm text-center py-4">No extra details for this task.</p>
                    )}
                  </div>
                </>
              )}
