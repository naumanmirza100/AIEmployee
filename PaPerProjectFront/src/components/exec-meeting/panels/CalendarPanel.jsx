...
// Receives plan state + handlers via props; owns only the "view as card" modal.
...
import {
  Loader2, CalendarDays, CalendarClock, Clock, CheckCircle2, Download, RefreshCw, ChevronRight, Maximize2,
} from 'lucide-react';
...
// Download the given week plan as a PDF. Kept as a standalone helper so both the
// inline view and the card pop-up can trigger it.
async function downloadWeekPlanPdf(weekPlan, toast) {
  try {
    ...
    // jsPDF's core fonts only cover Latin-1 — map Unicode punctuation to ASCII.
...
// The plan results (summary, recommendations, daily cards). Rendered both inline
// and inside the "view as card" pop-up, so it lives as its own component.
// `showDownload` hides the top Download button in the modal (the modal supplies
// its own header action).
export const WeekPlanResults = ({ weekPlan, toast, showDownload = true }) => {
  if (!weekPlan) return null;
  if (!weekPlan.daily_plans?.length && !weekPlan.weekly_summary) {
    return (
        ...
        <div className="flex justify-end">
          <HoverTip tip="Download this week's plan as PDF">
            <Button
              ...
              onClick={() => downloadWeekPlanPdf(weekPlan, toast)}
            >
...
export const CalendarPanel = ({
  ...
  // "View as card" pop-up. Auto-opens whenever a fresh plan finishes generating.
  const [planModalOpen, setPlanModalOpen] = useState(false);
  const prevPlanRef = useRef(weekPlan);
  useEffect(() => {
    const hadPlan = prevPlanRef.current;
    const hasContent = weekPlan && (weekPlan.daily_plans?.length > 0 || weekPlan.weekly_summary);
    // Open only when the plan reference changed to a non-empty plan (i.e. a new
    // generation), not on unrelated re-renders.
    if (weekPlan && weekPlan !== hadPlan && hasContent) {
      setPlanModalOpen(true);
    }
    prevPlanRef.current = weekPlan;
  }, [weekPlan]);
  ...
  const hasPlan = weekPlan && (weekPlan.daily_plans?.length > 0 || weekPlan.weekly_summary);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl p-5 space-y-4" style={CARD_STYLE}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {hasPlan && (
              <HoverTip tip="View this week's plan as a card">
                <Button size="sm" variant="ghost" onClick={() => setPlanModalOpen(true)} className="text-violet-300 hover:text-violet-200 gap-1.5">
                  <Maximize2 className="h-3.5 w-3.5" /> View as card
                </Button>
              </HoverTip>
            )}
            {weekPlan && (
              <HoverTip tip="Regenerate this week's plan">
                <Button size="sm" variant="ghost" onClick={refreshPlan} disabled={weekPlanLoading} className="text-white/40 hover:text-white">
        ...
        {/* Work-hours row — the window tasks get scheduled within. Disabled
            while a plan is being generated. */}
        <div className={`flex items-center justify-between rounded-xl px-4 py-3 bg-white/5 border border-white/10 transition-opacity ${weekPlanLoading ? 'opacity-50 pointer-events-none' : ''}`}>
          <div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <Select value={String(workStartHour)} disabled={weekPlanLoading} onValueChange={v => setWorkStartHour(Number(v))}>
            ...
            <Select value={String(workEndHour)} disabled={weekPlanLoading} onValueChange={v => setWorkEndHour(Number(v))}>
              <SelectTrigger className="h-8 w-[84px] bg-violet-500/10 border-violet-400/30 text-violet-100 text-xs hover:bg-violet-500/20 focus:ring-violet-500">
                <SelectValue />
              </SelectTrigger>
              <SelectContent className="bg-[#1a1030] border-violet-400/30 text-violet-100 max-h-56 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        {/* Settings row — disabled while a plan is being generated. */}
        <div className={`flex items-center justify-between rounded-xl px-4 py-3 bg-white/5 border border-white/10 transition-opacity ${weekPlanLoading ? 'opacity-50 pointer-events-none' : ''}`}>
          <div>
          ...
          <button
            ...
            className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none disabled:cursor-not-allowed ${
      ...
      {/* Inline plan results */}
      <WeekPlanResults weekPlan={weekPlan} toast={toast} />
        ...
        >
          <div className="flex items-center justify-between px-6 py-4 pr-14 border-b border-white/10 flex-shrink-0">
            <div className="flex items-center gap-2 min-w-0">
              <CalendarDays className="h-4 w-4 text-violet-400 flex-shrink-0" />
              <h3 className="text-white font-semibold truncate text-sm">
                AI Weekly Plan{weekPlan?.week_start ? ` — ${weekPlan.week_start}` : ''}
              </h3>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0 ml-4">
              <HoverTip tip="Download this week's plan as PDF">
                <Button size="sm" variant="ghost" onClick={() => downloadWeekPlanPdf(weekPlan, toast)}
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar px-6 py-5">
            <WeekPlanResults weekPlan={weekPlan} toast={toast} showDownload={false} />
          </div>
        </DialogContent>
      </Dialog>
    </div>
