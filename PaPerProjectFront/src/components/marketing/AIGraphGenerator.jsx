...
// Chart Components (same as recruitment)
...
const SimpleScatterPlot = ({ data, color = '#8b5cf6', height = 200, title }) => {
  if (!data || data.length === 0) return <div className="flex items-center justify-center h-48 text-muted-foreground">No data available</div>;
...
const SimpleHeatMap = ({ data, title }) => {
  if (!data?.rows || !data?.cols || !data?.values) return <div className="flex items-center justify-center h-48 text-muted-foreground">No data available</div>;
            ...
            {data.rows.map((row, rowIndex) => (
              <tr key={rowIndex}>
                ...
                {data.values[rowIndex]?.map((value, colIndex) => (
                  <td key={colIndex} className="p-1 text-center rounded" style={{ backgroundColor: getColor(value) }}>{value}</td>
                ))}
              </tr>
            ))}
          </tbody>
...
const chartTypeIcons = { bar: BarChart3, pie: PieChart, line: TrendingUp, area: Activity, scatter: Activity, heatmap: BarChart3 };
...
const renderChart = (chartData) => {
  ...
  switch (type) {
    ...
    case 'area': return <SimpleAreaChart data={data} color={color} title={title} />;
    case 'scatter': return <SimpleScatterPlot data={data} color={color} title={title} />;
    case 'heatmap': return <SimpleHeatMap data={data} title={title} />;
...
const AIGraphGenerator = () => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (savedPrompt) {
          ...
          setSearchParams((prev) => { const next = new URLSearchParams(prev); next.delete('runPromptId'); return next; });
        }
      } catch (e) { console.error('Error running prompt from URL:', e); }
  ...
  const handleGenerate = async () => {
    if (!prompt.trim()) {
      toast({ title: 'Empty prompt', description: 'Please enter a prompt to generate a graph', variant: 'destructive' });
      return;
    }
    try {
      setGenerating(true);
  ...
  const handleSavePrompt = async () => {
    if (!saveTitle.trim()) {
      toast({ title: 'Title required', description: 'Please enter a title for this prompt', variant: 'destructive' });
      return;
    }
    try {
      setSaving(true);
      const tags = saveTags.split(',').map(t => t.trim()).filter(t => t);
      const response = await saveGraphPrompt({
        title: saveTitle,
        prompt: prompt,
        tags: tags,
      ...
      if (response.status === 'success') {
        toast({ title: 'Prompt saved', description: 'You can find it in your saved prompts' });
      ...
      toast({ title: 'Save failed', description: error.message || 'Failed to save prompt', variant: 'destructive' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleRunSavedPrompt = async (savedPrompt) => {
    ...
    setTimeout(async () => {
      try {
        ...
        toast({ title: 'Generation failed', description: 'Failed to generate graph from saved prompt', variant: 'destructive' });
      } finally {
        setGenerating(false);
      }
    }, 100);
  ...
  const handleDeletePrompt = async (promptId) => {
    try {
      ...
      toast({ title: 'Delete failed', description: 'Failed to delete prompt', variant: 'destructive' });
    }
  };

  const handleToggleFavorite = async (promptId, currentFavorite) => {
    try {
      const response = await toggleGraphPromptFavorite(promptId, { is_favorite: !currentFavorite });
      if (response.status === 'success') {
  ...
  const handleToggleDashboard = async (savedPrompt) => {
    try {
      const response = await toggleGraphPromptDashboard(savedPrompt.id);
      if (response.status === 'success' && response.data) {
        setSavedPrompts(prev => prev.map(p => (p.id === savedPrompt.id ? { ...p, tags: response.data.tags ?? p.tags } : p)));
        toast({
          title: response.data.on_dashboard ? 'Added to dashboard' : 'Removed from dashboard',
          description: response.data.on_dashboard ? 'Card will appear on marketing dashboard' : '',
        });
      }
    } catch (error) {
      console.error('Error toggling dashboard:', error);
      toast({ title: 'Failed', description: 'Could not update dashboard', variant: 'destructive' });
    }
  };
    ...
    <div className="space-y-4 sm:space-y-6 w-full">
      <div>
        ...
        <p className="text-xs sm:text-sm text-muted-foreground">
          Generate marketing visualizations using natural language. Describe what you want to see and AI will create the chart from your campaigns and email data.
        </p>
      </div>
          ...
          <Card>
            <CardHeader className="p-4 sm:p-6">
              ...
              <CardDescription className="text-xs sm:text-sm">
                Describe the chart you want (campaigns, open rates, leads, replies, etc.)
              </CardDescription>
            </CardHeader>
            <CardContent className="p-4 sm:p-6 pt-0 space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="e.g., Show campaigns by status as a pie chart"
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {examplePrompts.slice(0, 4).map((example, index) => (
                    <Button key={index} variant="outline" size="sm" className="text-xs h-auto py-1.5 px-2" onClick={() => setPrompt(example)}>
                      {example}
                    </Button>
                  ))}
                </div>
              </div>
              <Button onClick={handleGenerate} disabled={generating || !prompt.trim()} className="w-full sm:w-auto">
                {generating ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Generating...</> : <><Sparkles className="h-4 w-4 mr-2" /> Generate Graph</>}
              </Button>
            </CardContent>
          </Card>
          ...
          {generatedChart && !generating && (
            <Card>
              <CardHeader className="p-4 sm:p-6 flex flex-row items-start justify-between">
                <div>
                  <CardTitle className="text-base sm:text-lg flex items-center gap-2">
                    {chartTypeIcons[generatedChart.type] && React.createElement(chartTypeIcons[generatedChart.type], { className: 'h-5 w-5' })}
                    {generatedChart.title || 'Generated Chart'}
                  </CardTitle>
                  {chartInsights && <CardDescription className="text-xs sm:text-sm mt-1">{chartInsights}</CardDescription>}
                </div>
              ...
              <CardContent className="p-4 sm:p-6 pt-0">
                <div className="bg-muted/30 rounded-lg p-4 sm:p-6">{renderChart(generatedChart)}</div>
              </CardContent>
            </Card>
          )}
        </TabsContent>
            ...
            <CardHeader className="p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                  ...
                  <CardDescription className="text-xs sm:text-sm">Your saved graph prompts for quick access</CardDescription>
                </div>
            ...
            <CardContent className="p-4 sm:p-6 pt-0 space-y-4">
              ...
              {loadingPrompts ? (
                ...
                <div className="py-8 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">{searchQuery || filterFavorites ? 'Try adjusting your filters' : 'Generate a graph and save it to see it here'}</p>
                </div>
              ) : (
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {savedPrompt.tags?.slice(0, 3).map((tag, i) => <Badge key={i} variant="outline" className="text-[10px]">{tag}</Badge>)}
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem onClick={() => handleToggleDashboard(savedPrompt)}>
                                  ...
                                  {isGraphPromptOnDashboard(savedPrompt) ? 'Remove from dashboard' : 'Add to dashboard'}
                                </DropdownMenuItem>
                                <DropdownMenuSeparator />
        ...
        <DialogContent className="w-[95vw] max-w-md">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input id="save-title" value={saveTitle} onChange={(e) => setSaveTitle(e.target.value)} placeholder="e.g., Campaigns by Status" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="save-tags" value={saveTags} onChange={(e) => setSaveTags(e.target.value)} placeholder="e.g., campaigns, status, overview" />
            </div>
          </div>
          <DialogFooter>
