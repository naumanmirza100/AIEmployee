...
def _env_flag(name):
    """True / False from an environment variable, or None when unset or unrecognised."""
...
# Encryption of stored API keys (core/crypto_utils.py). Must be the same on every
# machine using this database. Until FIELD_ENCRYPTION_KEY is set, stored keys are
# encrypted with a key derived from SECRET_KEY, as before.
...
if DEBUG:
    ...
    CORS_ALLOWED_ORIGINS = [o.strip().rstrip('/') for o in cors_origins_env.split(',') if o.strip()]
    # The deployment's own React app, so setting FRONTEND_URL is enough.
    _frontend_origin = (os.getenv('FRONTEND_URL') or '').strip().rstrip('/')
    if _frontend_origin and _frontend_origin not in CORS_ALLOWED_ORIGINS:
        CORS_ALLOWED_ORIGINS.append(_frontend_origin)

# --------------------
# HTTPS / cookie hardening (only when DEBUG is off)
# --------------------
if not DEBUG:
    # Session and CSRF cookies only over HTTPS. The React app authenticates
    # with a token header, so this affects the Django admin and the old
    # server-rendered pages. Set DJANGO_SECURE_COOKIES=0 for a plain-HTTP server.
    SESSION_COOKIE_SECURE = _env_flag('DJANGO_SECURE_COOKIES') is not False
    CSRF_COOKIE_SECURE = SESSION_COOKIE_SECURE
    # Behind a proxy that terminates HTTPS (nginx, a hosting panel, …) Django
    # sees plain HTTP; the proxy's X-Forwarded-Proto header says what it was.
    # Only enable when that proxy really sets the header.
    if _env_flag('DJANGO_BEHIND_HTTPS_PROXY'):
        SECURE_PROXY_SSL_HEADER = ('HTTP_X_FORWARDED_PROTO', 'https')
    # Opt-in: a redirect loop or a wrong HSTS value locks users out, so these
    # are switched on per deployment once HTTPS is confirmed working.
    SECURE_SSL_REDIRECT = bool(_env_flag('DJANGO_SSL_REDIRECT'))
    _hsts = os.getenv('DJANGO_HSTS_SECONDS', '').strip()
...
# NOTE: DatabaseScheduler queries the database and can cause REGEXP_LIKE errors with SQL Server
# Using default scheduler (from CELERY_BEAT_SCHEDULE) to avoid database queries
# CELERY_BEAT_SCHEDULER = 'django_celery_beat.schedulers:DatabaseScheduler'  # Disabled to avoid REGEXP_LIKE errors
...
REST_FRAMEWORK = {
    ...
    'PAGE_SIZE': 20,
    # The browsable HTML API explorer only while DEBUG; a deployed API speaks JSON.
    ...
    'DEFAULT_THROTTLE_RATES': {
        # PM chat endpoints, one shared budget per user (PMLLMThrottle).
        'pm_llm': '30/hour',
        # Other PM AI tools, per user per endpoint (PMLLMToolThrottle).
        'pm_llm_tool': '60/hour',
        'pm_crud': '1000/hour',
